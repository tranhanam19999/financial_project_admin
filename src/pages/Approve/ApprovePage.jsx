...
import { deleteTran } from '../../api'

const TableBody = ({setModalShow, setOptionType, setCurrentItem,page,listtrans}) => {
    //const listtrans = useSelector(state => {return state.listtrans})
    let listTranPaginated = (listtrans == []) ? [] : _.chunk(listtrans,9)
        ...
        return(<tr key= { index }>
            ...
            <td style={{textAlign:"center"}}>
                ...
                ? <div className="row">
                    ...
                    <button onClick={() => {setModalShow(true); setOptionType('update'); setCurrentItem(val)}}
                            className="btn btn-primary col-md-4 mr-2">
                        <span className="icon text-center">
                            <i className="fas fa-check-circle"></i>
                        </span>
                    </button>
                    <button onClick={() => {deleteTran(val) ; setCurrentItem(val)}}
                ...
                : <button onClick={() => {alert(val._id); setCurrentItem(val)}} className="btn btn-info col-md-3">
                    <span className="icon text-center">
...
const ApprovePage = () => {
    ...
    return (
        ...
        <div id="content">
            ...
            <div className="container-fluid">
                ...
                <div className="card shadow mb-4">
                    ...
                    <h6 className="m-0 font-weight-bold text-primary">Approve Page</h6>
                    </div>
                                ...
                                <ul className="pagination">
                                    {(() => {
                                            return (listtrans == []) ? <></> : pageArray.map((val,i) => {
                                                    return <li
                                                            key = {i}
                                                            className={"page-item " + (i == active ? " active" : null)}
                                                            onClick={() => setPage(val)}>
                                                        <a className="page-link" onClick={() => setActive(i)}>{val}</a>
                                                    </li>
                                            })
                                    })()}
                                </ul>
        ...
        {!currentItem ? <></> :
        <ModalInit collection = "approve" show={modal} onHide={() => setModalShow(false)} optionType={optionType} item={currentItem}/>}
        </>
        )
}
